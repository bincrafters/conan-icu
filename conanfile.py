...
class IcuConan(ConanFile):
    ...
    def build(self):
        src_path = os.path.join(self.name, 'source')

        makedata_file = os.path.join(src_path, 'data', 'makedata.mak')
        tools.replace_in_file(makedata_file,
    ...
    def package(self):
        if self.settings.os == 'Windows':
            ...
            if self.options.with_io:
                libs.append('io')
            for lib in libs:
                self.copy(pattern="*icu{0}.dylib".format(lib), dst="lib", src="{0}/lib".format(install_path),
                          keep_path=False)
                self.copy(pattern="*icu{0}.so".format(lib), dst="lib", src="{0}/lib".format(install_path),
                          keep_path=False)
